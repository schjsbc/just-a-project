...
rule minimap_mapping:
    input:
        genome = rules.minimap_index.output,
        reads=get_trimmed_reads,
    output:
        "mapped_reads/{sample}.bam"
    shell:
        "minimap2 -Y -t  -ax sr {input.genome} {input.reads} > output.sam | samtools view -b -o {output}"

rule mark_duplicates:
    input:
        "results/mapped/{sample}-{unit}.sorted.bam" if config["processing"]["minimap"] else "results/mapped/{sample}-{unit}.sorted.bam",
    output:
